...
% (Uncomment the one you want; comment out the other with a %)
% =========================================================================
% 1. LOCAL DEVELOPMENT:
%\newcommand{\pzoomserver}{http://127.0.0.1:4000}
%\renewcommand{\pzpdfbase}{http://127.0.0.1:4000/assets/pdfs/entry-0009/}
\renewcommand{\pzpdfbase}{https://proofzoom.org/assets/pdfs/entry-0009/}
% 2. GITHUB PAGES PRODUCTION:
% \newcommand{\pzoomserver}{https://YOUR-USERNAME.github.io/YOUR-REPO}
\hypersetup{hidelinks}
\begin{document}

\begin{center}
{\normalsize \textbf{Entry-0009}}\\[1.0em]
{\Large \textbf{Ordinal Numbers and the Shape of Order}\\[0.4em]
\large \textit{Part I: Foundations and Representation}}\\[0.8em]

\textbf{Area:} Foundations \quad $\bullet$ \quad
\textbf{Subarea:} Set Theory\\[0.35em]
\textbf{Difficulty:} UG-Upper \quad $\bullet$ \quad
\textbf{Published:} 2026-06-30 \quad $\bullet$ \quad
\textbf{Updated:}
\end{center}
...
% The essay text directly starts below with your first section
...
\section{Order Type: Forgetting Names, Remembering Positions}
...
Mathematically, two ordered sets have the same order type if there is a bijection between them that preserves order. Such a bijection is called an \textbf{order-isomorphism}. 
...
\begin{proposition}[Rigidity of Well-Orderings]
Let \((S,<)\) and \((S',<')\) be well-ordered sets. If there is an order-isomorphism from $S$ to $S'$, then it is unique.
\end{proposition}

\begin{proof}
Suppose $\phi, \psi:S\to S'$ are two distinct order-isomorphisms. We shall prove that \(\phi=\psi\).
...
\pzoom[Goal={Prove that \(\psi(a)\) escapes the image of \(\phi\)}]{/assets/pdf/entry-0009/entry-0009-1.pdf}{}

So $\psi(a) \notin \text{im}(\phi)$. This directly contradicts the fact that $\phi$ is a surjective order-isomorphism onto $S'$. Thus, our initial assumption that a point of disagreement exists must be false, proving that $\phi = \psi$.
\end{proof}
...
\section{The Tempting Definition and Why It Fails}
...
\section{von Neumann's Idea: Each Position Is the Set of Earlier Positions}
...
We now formalize this construction. A set $\alpha$ is called \textbf{transitive} if every element of $\alpha$ is also a subset of $\alpha$:
\[
x\in\alpha \quad \Longrightarrow \quad x\subseteq\alpha.
\]
Equivalently, if $x\in y\in\alpha$, then $x\in\alpha$.

\begin{definition}[von Neumann ordinal]
A \textbf{von Neumann ordinal} is a set $\alpha$ satisfying two properties:
\begin{enumerate}[label=(\textnormal{Or\arabic*})]
\item $\alpha$ is transitive.
\item The membership relation $\in$ well-orders $\alpha$.
\end{enumerate}
\end{definition}
...
\begin{enumerate}
\item \textbf{Members of ordinals are ordinals (Section 7).} We show that ordinals are internally coherent; opening an ordinal reveals only smaller ordinals.
\item \textbf{Trichotomy: Any two ordinals are comparable (Section 8).} We establish that the class of ordinals is perfectly linear and non-branching.
\item \textbf{Transitive sets of ordinals are ordinals (Section 9).} We provide a practical test for ordinality, demonstrating that a gapless set of ordinals is itself an ordinal.
\item \textbf{The Representation Theorem (Section 10).} We prove that every abstract well-ordered set is order-isomorphic to exactly one canonical von Neumann ordinal.
\end{enumerate}
...
\section{Members of Ordinals Are Ordinals}
...
\begin{enumerate}[label=(\roman*)]
    \item $\beta$ is transitive.
    \item The membership relation $\in$ well-orders $\beta$.
\end{enumerate}

\begin{proof}
Let $\alpha$ be an ordinal and let $\beta \in \alpha$. 

\medskip
\noindent\textbf{Step 1: The membership relation well-orders $\beta$.}\\
Since $\alpha$ is an ordinal, the relation $\in$ well-orders $\alpha$. Because $\alpha$ is transitive, $\beta \in \alpha$ implies that $\beta \subseteq \alpha$. 
...
\medskip
\noindent\textbf{Step 2: $\beta$ is transitive.}\\
To show that $\beta$ is transitive, we must establish that any element belonging to $\beta$ is also a subset of $\beta$. By leveraging the transitivity of the parent ordinal $\alpha$ and the properties of its strict total membership ordering, the relation $x \in y \in \beta$ forces $x \in \beta$.

\pzoom[Goal={Verify that \(\beta\) is a transitive set}]{/assets/pdf/entry-0009/entry-0009-2.pdf}{}

\medskip
\noindent\textbf{Conclusion:}\\
Since $\beta$ is transitive and well-ordered by $\in$, it is a von Neumann ordinal.
\end{proof}
...
\section{Trichotomy: Any Two Ordinals Are Comparable}
...
\begin{lemma}[Structure of Transitive Subsets]
A transitive subset of an ordinal is an initial segment of that ordinal.
\end{lemma}
...
\begin{proof}
Let $\alpha$ be an ordinal and $\gamma \subseteq \alpha$ be a transitive subset such that $\gamma \neq \alpha$.
\begin{enumerate}
\item \textbf{Find the least element missing from $\gamma$:} Since $\gamma$ is a proper subset of $\alpha$, the relative complement $\alpha \setminus \gamma$ is non-empty. Because $\alpha$ is well-ordered by $\in$, the set $\alpha \setminus \gamma$ must contain a unique least element. Let us call this element $\xi_0$.
\item \textbf{Show $\gamma \subseteq \xi_0$:} \\
Let $x \in \gamma$. Since $x$ and $\xi_0$ are both elements of the ordinal $\alpha$, they must be comparable under membership. 
\begin{itemize}
    \item If $x = \xi_0$, then $\xi_0 \in \gamma$, contradicting the fact that $\xi_0 \in \alpha \setminus \gamma$.
    \item If $\xi_0 \in x$, then because $\gamma$ is a transitive set and $x \in \gamma$, it follows that $\xi_0 \in \gamma$, yielding the same contradiction.
    \item Therefore, by elimination, it must be that $x \in \xi_0$. Thus, $\gamma \subseteq \xi_0$.
\end{itemize}
\item \textbf{Show $\xi_0 \subseteq \gamma$:} \\
Let $y \in \xi_0$. Since $\alpha$ is transitive and $\xi_0 \in \alpha$, we know $y \in \alpha$. Because $y \in \xi_0$, we have $y < \xi_0$. Since $\xi_0$ was defined as the least element of $\alpha \setminus \gamma$, any element in $\alpha$ strictly smaller than $\xi_0$ cannot be in $\alpha \setminus \gamma$. Therefore, $y$ must belong to $\gamma$, meaning $\xi_0 \subseteq \gamma$.
\end{enumerate}
Since $\gamma \subseteq \xi_0$ and $\xi_0 \subseteq \gamma$, we have $\gamma = \xi_0$. Because $\xi_0 \in \alpha$, $\gamma$ is an element of $\alpha$, meaning it is precisely an initial segment.
\end{proof}

\begin{lemma}[Intersection of Ordinals]
For any two ordinals $\alpha$ and $\beta$, their intersection $\gamma = \alpha \cap \beta$ is also an ordinal, and $\gamma$ is an initial segment of both $\alpha$ and $\beta$.
\end{lemma}

\begin{proof}
If $x \in \gamma$, then $x \in \alpha$ and $x \in \beta$. Since $\alpha$ and $\beta$ are transitive, any $y \in x$ must belong to both $\alpha$ and $\beta$, so $y \in \gamma$, proving $\gamma$ is transitive. Furthermore, since $\gamma \subseteq \alpha$ and $\alpha$ is well-ordered by $\in$, $\gamma$ inherits this well-ordering. Thus, $\gamma$ is an ordinal. Since $\gamma$ is a transitive subset of both $\alpha$ and $\beta$, Lemma 1 implies it is an initial segment of both.
\end{proof}

\begin{lemma}[The Proper Subset Property]
For any ordinals $\mu$ and $\nu$, if $\mu \subseteq \nu$ and $\mu \neq \nu$, then $\mu \in \nu$.
\end{lemma}
...
\begin{theorem}[Trichotomy for Ordinals]
For any two ordinals $\alpha$ and $\beta$, exactly one of the following holds:
\[
...
If $\gamma = \beta$, then $\beta \subseteq \alpha$, meaning either $\beta = \alpha$ or $\beta \in \alpha$.

\medskip
Assume for contradiction that $\gamma \neq \alpha$ and $\gamma \neq \beta$. By applying our structural lemmas to this joint proper subset condition, we force a membership configuration that directly violates the Axiom of Regularity.

\pzoom[Goal={Exhaust the contradiction where \(\gamma\) is a proper subset of both}]{/assets/pdf/entry-0009/entry-0009-3.pdf}{}

\end{proof}

\section{Transitive Sets of Ordinals}
...
\begin{proposition}
Let $X$ be a set of ordinals. Then $X$ is an ordinal if and only if $X$ is transitive.
\end{proposition}

\begin{proof}
If $X$ is an ordinal, then $X$ is transitive by definition. This establishes the forward direction.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item The membership relation $\in$ well-orders $X$.
\end{enumerate}
...
\medskip
\noindent\textbf{Step 1: The relation $\in$ totally orders $X$.}\\
Let $\alpha, \beta \in X$. Because $X$ is a set of ordinals, both $\alpha$ and $\beta$ are ordinals. By the Trichotomy Theorem, exactly one of the following conditions must hold:
\[
\alpha \in \beta, \qquad \beta \in \alpha, \qquad \alpha = \beta.
\]
...
\medskip
\noindent\textbf{Step 2: Every nonempty subset of $X$ possesses a least element.}\\
Let $S \subseteq X$ be a nonempty subset. We must demonstrate that $S$ contains an element which precedes all other members under the relation $\in$.
...
\medskip
To verify this, we select any distinct element $\gamma \in S$. By utilizing Trichotomy on these two ordinals, assuming $\gamma \in \alpha$ forces an intersection with $S$ that directly violates our Regularity condition, leaving $\alpha \in \gamma$ as the only logical outcome.

\pzoom[Goal={Exhaust the Regularity contradiction to prove \(\alpha\) is minimal}]{/assets/pdf/entry-0009/entry-0009-4.pdf}{}

\medskip
\noindent\textbf{Conclusion:}\\
Every nonempty subset of $X$ has a least element under $\in$. Consequently, $\in$ well-orders $X$. Since $X$ is both transitive and well-ordered by membership, it is a von Neumann ordinal.
\end{proof}
...
\section{The Representation Theorem for Well-Orderings}
...
\begin{theorem}[Representation Theorem for Well-Orderings]
...
\begin{lemma}
No well-ordered set is order-isomorphic to one of its proper initial segments.
\end{lemma}

\begin{proof}(Proof of Lemma)
Let $(W,<)$ be a well-ordered set, and let $I_b = \{w \in W : w < b\}$ be the proper initial segment of $W$ determined by some $b \in W$. Suppose for contradiction that there exists an order-isomorphism $h: W \to I_b$.
...
h(h(d)) < h(d)
\]
This inequality implies that the element $h(d)$ satisfies the defining condition of $D$. Thus, $h(d) \in D$. However, because $h(d) < d$, this directly contradicts our selection of $d$ as the \textit{least} element of $D$. 

By contradiction, no such order-isomorphism can exist.
\end{proof}

With this lemma in hand, we can cleanly prove the uniqueness and existence of our ordinal representatives.

\begin{proof}[Proof of the Representation Theorem]
We divide the proof into two parts: Uniqueness and Existence.

\medskip
\noindent\textbf{Part 1: Uniqueness}\\
Suppose a well-ordered set $A$ is order-isomorphic to two distinct ordinals, $\alpha$ and $\beta$. By composing the isomorphisms, it follows that $\alpha$ and $\beta$ must be order-isomorphic to each other.

By the Trichotomy Theorem, exactly one of the following holds: $\alpha \in \beta$, $\beta \in \alpha$, or $\alpha = \beta$. 
\begin{itemize}
    \item If $\alpha \in \beta$, then because $\beta$ is transitive, $\alpha$ is a proper subset of $\beta$ consisting precisely of the elements preceding $\alpha$. Thus, $\alpha$ is a proper initial segment of $\beta$. By our Lemma, $\beta$ cannot be order-isomorphic to one of its proper initial segments, ruling out $\alpha \in \beta$.
    \item By an identical symmetric argument, $\beta \in \alpha$ is impossible.
\end{itemize}
Therefore, we are forced to conclude that $\alpha = \beta$, confirming that the representing ordinal is unique.

\medskip
\noindent\textbf{Part 2: Existence}\\
Let $(A,<)$ be an arbitrary well-ordered set. For each $x \in A$, let $A_x = \{y \in A : y < x\}$ denote the initial segment of $A$ determined by $x$. 

Let us say an element $x \in A$ is \textit{good} if its initial segment $A_x$ is order-isomorphic to some ordinal. By our uniqueness proof, if such an ordinal exists, it is unique; we denote it by $\alpha_x$. We will show by well-ordered induction on $A$ that every element of $A$ is good.
...
\medskip
Finally, we define the full ordinal representative for the entire set $A$: $\alpha = \{\alpha_x : x \in A\}$. Replicating the exact structural argument used for $\beta$, $\alpha$ is a transitive set of ordinals and is therefore an ordinal.

\pzoom[Goal={Verify that \(\alpha\) is a transitive set of ordinals}]{/assets/pdf/entry-0009/entry-0009-5.pdf}{}

The map $G : A \to \alpha$ given by $G(x) = \alpha_x$ yields a total order-isomorphism from $A$ onto $\alpha$.
Thus, $A$ is order-isomorphic to a unique ordinal.
\end{proof}
...
\bibitem{neumann1923}
J. von Neumann, \emph{Zur Einführung der transfiniten Zahlen}, Acta Litt. Acad. Sc. Szeged, 1 (1923), 199–208.

\bibitem{neumann1928}
J. von Neumann, \emph{Die Axiomatiserung der Mengenlehre}, Mathematische Zeitschrift, 27 (1928), 669–752.

\bibitem{halmos1974}
P. R. Halmos, \emph{Naive Set Theory}, Springer, 1974.

\bibitem{kunen1980}
K. Kunen, \emph{Set Theory: An Introduction to Independence Proofs}, North-Holland, 1980.

\bibitem{jech2003}
T. Jech, \emph{Set Theory}, Springer Monographs in Mathematics, 2003.

\bibitem{proofzoom0007}
ProofZoom Entry-0007, \emph{Zorn’s Lemma, Well-Ordering, and the Axiom of Choice}, ProofZoom (2026).

\end{thebibliography}

\vspace{1cm}
\noindent \textbf{Cite as:} ProofZoom Entry-0009, \emph{Ordinal Numbers and the Shape of Order (Part I: Foundations and Representation)}, ProofZoom (2026).
% =========================================================================
% Appendix: ProofZoom Deep Dives
% =========================================================================

...
The diagram illustrates a line through $a$ and $b$ having direction vector $b-a$ and parametric form $a+(b-a)t$.
</div>

<div class="pdf-diagram-note">
<strong>Figure 1.</strong> Diagram omitted in the web version.  
<a href="/assets/pdfs/entry-0002-curves-tangents-angles.pdf#page=3" target="_blank">
View in PDF
</a>.
</div>
...
$$
\theta = \operatorname{Arg}(w) - \operatorname{Arg}(v)=\operatorname{Arg}\!\left(\frac{w}{v}\right).
$$
...
A curve in the complex plane is a continuous complex-valued function $z(t)$ defined on an interval $[\alpha, \beta]$ of real numbers.
Let $t_0 \in (\alpha,\beta)$. Let $z(t_0 - h), z(t_0 + h)$ be points near $z(t_0)=z_0$.
Suppose $z'(t_0)$ exists and $z'(t_0) \neq 0$. 
...
=
\left\{
z : z = z_0 + s\,\frac{z(t_0 + h) - z_0}{h}, \quad s \ge 0
\right\}.
$$
...
$$
T(z_0) = \{ z : z = z_0 + s\,z'(t_0), \quad s \ge 0 \}
$$
...
<div class="pdf-diagram-note">
<strong>Figure 2.</strong> Diagram omitted in web version.
<a href="/assets/pdfs/entry-0002-curves-tangents-angles.pdf#page=2" target="_blank">
View in PDF
</a>.
</div>
...
**Remark 4.1.** A tangent may still exist at a point where $z'(t_0) = 0$, but then it is determined by higher-order behavior rather than by the first derivative (see [Section 9](#sec-geomeaning)).
...
Hence
<div class="eq-block" id="eq-5-1">
  <div class="eq-center">
  $$
  \widetilde{z}'(t_0) = \frac{z'(t_0)}{\phi'(t_0)}
  $$
  </div>
  <div class="eq-number">(5.1)</div>
</div>
...
$$
\angle(C_1, C_2) = \operatorname{Arg}(z_2'(t_0)) - \operatorname{Arg}(z_1'(t_0))= \operatorname{Arg}\!\left(\frac{z_2'(t_0)}{z_1'(t_0)}\right).
$$
...
f(z(t_0))
+
f'(z(t_0))\bigl(z(t_0 + h) - z(t_0)\bigr)
+
o\bigl(|z(t_0 + h) - z(t_0)|\bigr).
$$
...
f(z_0)
+
\frac{f^{(m)}(z_0)}{m!}(z - z_0)^m
+
o(\lvert z - z_0\rvert ^m).
$$

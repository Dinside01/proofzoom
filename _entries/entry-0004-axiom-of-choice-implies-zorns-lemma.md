...
Since $A$ is well-ordered, any two elements of $A$ are comparable; therefore $I(A,x)$ is a chain in $X$. Hence $I(A,x)$ lies in the domain of the choice function $f$, so $f(I(A,x))$ is well-defined—that is, it is a specific element of $X$ strictly greater than every element of $I(A,x)$.
